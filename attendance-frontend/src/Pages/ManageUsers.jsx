...
// ── Password Store: username → plain password (persists across renders) ──
const PWD_KEY = "manage_users_passwords";
const loadPwdStore  = () => JSON.parse(localStorage.getItem(PWD_KEY) || "{}");
const savePwdStore  = (store) => localStorage.setItem(PWD_KEY, JSON.stringify(store));
const savePassword  = (username, pwd) => {
  const store = loadPwdStore();
  store[username] = pwd;
  savePwdStore(store);
};
const getPassword   = (username) => loadPwdStore()[username] || "";
...
const emptyUser = () => ({
  name: "", role: "Staff", username: "", password: "", department: ""
});
    ...
    {Object.entries(ROLE_CONFIG).map(([role, cfg]) => {
      ...
      return (
        <label key={role} style={{
          ...
          <input type="radio" name="role" value={role} checked={selected}
            onChange={() => onChange(role)} style={{ display: "none" }} />
          <span style={{ fontSize: "16px" }}>{cfg.emoji}</span>
          {role}
          {selected && <span style={{ width:8,height:8,borderRadius:"50%",background:cfg.color,display:"inline-block" }} />}
        </label>
      );
    })}
  </div>
  ...
  return (
    <div className="input-group shadow-sm">
      ...
      <button className="btn btn-outline-secondary" type="button"
        onClick={() => setShow(!show)} style={{ fontSize: "16px" }}>
        {show ? "🙈" : "👁️"}
      </button>
    </div>
  );
...
const PasswordDisplay = ({ password }) => {
  ...
  if (!val) {
    return (
      <span style={{
        ...
        background:"#fef9c3", borderRadius:"6px", padding:"2px 10px",
  ...
  return (
    <span
      ...
      style={{
        fontFamily:"monospace", fontSize:"13px",
        background:"#f1f5f9", borderRadius:"6px", padding:"2px 10px",
    ...
    >
      {show ? val : "••••••••"}
      <span style={{ fontSize:12, color:"#94a3b8" }}>{show ? "🙈" : "👁️"}</span>
    </span>
  );
};
...
export default function ManageUsers() {
  ...
  const loadCollegeNames = async () => {
    try {
      ...
      const hods  = (hodRes.data  || []).map(h => ({ name:h.name, role:"HOD",   department:h.department }));
      const staff = (staffRes.data|| []).map(s => ({ name:s.name, role:"Staff", department:s.department }));
      setCollegeNames([...hods, ...staff]);
    } catch (err) { console.error("Failed to load college names:", err); }
  };
  ...
  const loadUsers = async () => {
    ...
    try {
      ...
      const pwdStore = loadPwdStore(); // ← get saved passwords

      const formatted = res.data.map((u, i) => {
        // Try API fields first, fallback to our localStorage store
        const pwd =
          u.password       ||
          u.plainPassword  ||
          u.plain_password ||
          u.pass           ||
          u.pwd            ||
          pwdStore[u.username] || // ← our saved password
          "";
  ...
  const handleNameInput = (val) => {
    ...
    const matches = collegeNames.filter(u => u.name.toLowerCase().startsWith(val.toLowerCase()));
  ...
  const handleNameBlur = () => {
    setTimeout(() => {
      const isValid = collegeNames.find(u => u.name.toLowerCase() === newUser.name.toLowerCase());
  ...
  const selectSuggestion = (s) => {
    setNewUser({ ...newUser, name:s.name, role:s.role, department:s.department || "" });
    setNameSuggestions([]);
    setShowDropdown(false);
    setNameError("");
  };
  ...
  const handleAdd = async () => {
    ...
    try {
      ...
      // ✅ Save password to localStorage so it shows in table
      savePassword(newUser.username, newUser.password);
  ...
  const handleUpdate = async () => {
    try {
      ...
      if (editData.newPassword) {
        savePassword(editData.username, editData.newPassword);
      }
  ...
  const handleDelete = async (username) => {
    ...
    try {
      ...
      // Remove from password store too
      const store = loadPwdStore();
            ...
            <div className="modal-dialog modal-dialog-centered" onClick={(e) => e.stopPropagation()}>
              <div className="modal-content border-0 shadow-lg" style={{ borderRadius:"20px", overflow:"hidden" }}>
                  ...
                  <div className="rounded-3 p-3" style={{ background:"#f8fafc" }}>
                    ...
                    <div className="d-flex justify-content-between align-items-center py-2">
                      ...
                      <div className="d-flex align-items-center gap-2"
                        ...
                        <span style={{ fontSize:"13px", fontWeight:"600", color:selectedUser.active?"#15803d":"#dc2626" }}>
                ...
                <div className="modal-footer border-0 d-flex justify-content-between px-4 pb-4">
                  <button className="btn btn-danger" onClick={() => handleDelete(selectedUser.username)}>🗑 Delete</button>
                  <div className="d-flex gap-2">
                    ...
                    <button className="btn text-white fw-bold px-3"
                      ...
                      onClick={() => { setEditData({ ...selectedUser, newPassword:"" }); setShowViewModal(false); setShowEditModal(true); }}>
                      ✏ Edit
                    </button>
                  </div>
                </div>
          ...
          <div className="modal-dialog modal-dialog-centered">
            <div className="modal-content border-0 shadow-lg p-3" style={{ borderRadius:"20px" }}>
              ...
              <div className="modal-body">
                ...
                <input className="form-control mb-3 shadow-sm bg-light" value={editData.username} disabled />
                ...
                <RoleRadioGroup value={editData.role} onChange={(role) => setEditData({ ...editData, role })} />
                ...
                <div className="mt-3">
                  ...
                  <div className="d-flex align-items-center gap-2" style={{ cursor:"pointer", width:"fit-content" }}
                    onClick={() => setEditData({ ...editData, active:!editData.active })}>
                    <StatusDot active={editData.active} />
                    <span style={{ fontSize:"14px", fontWeight:"600", color:editData.active?"#15803d":"#dc2626" }}>
              ...
              <div className="modal-footer border-0 d-flex justify-content-between">
                <button className="btn btn-outline-danger" onClick={() => handleDelete(editData.username)}>Delete</button>
                <div className="d-flex gap-2">
          ...
          <div className="modal-dialog modal-dialog-centered">
            <div className="modal-content border-0 shadow-lg p-3" style={{ borderRadius:"20px" }}>
              ...
              <div className="modal-body">
                ...
                <div className="position-relative mb-1" ref={dropdownRef}>
                  ...
                  {showDropdown && (
                    <div className="position-absolute w-100 shadow-lg"
                      style={{ top:"100%",left:0,zIndex:9999,background:"white",borderRadius:"10px",border:"1px solid #e2e8f0",overflow:"hidden",marginTop:"4px" }}>
                      {nameSuggestions.map((s, i) => {
                        const cfg = ROLE_CONFIG[s.role] || {};
                        return (
                          <div key={i} className="d-flex align-items-center gap-2 px-3 py-2"
                            style={{ cursor:"pointer" }}
                            onMouseEnter={(e) => (e.currentTarget.style.background = "#f1f5f9")}
                            onMouseLeave={(e) => (e.currentTarget.style.background = "white")}
                            onMouseDown={() => selectSuggestion(s)}>
                            <div style={{ width:34,height:34,borderRadius:"50%",background:cfg.color,color:"white",display:"flex",alignItems:"center",justifyContent:"center",fontWeight:"bold",fontSize:"13px",flexShrink:0 }}>
                              {getInitials(s.name)}
                            </div>
                            <div>
                ...
                {nameError && (
                  <div className="mt-1 mb-2" style={{ fontSize:"13px",color:"#dc2626",background:"#fee2e2",borderRadius:"8px",padding:"6px 12px" }}>
                    {nameError}
                  </div>
                )}
                ...
                <label className="form-label fw-semibold d-block mb-2">Role</label>
                <RoleRadioGroup value={newUser.role} onChange={(role) => setNewUser({ ...newUser, role })} />
