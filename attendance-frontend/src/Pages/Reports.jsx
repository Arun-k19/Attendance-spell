...
const ALL_DEPTS   = ["CSE","IT","ECE","MECH","CIVIL","EEE"];
...
const getRoleInfo = () => {
  ...
  if (role === "admin") return { role:"admin", allowedDepts:ALL_DEPTS, allowedYearsByDept:null, staffSubjects:null };
  ...
  const map = {};
  subjects.forEach(({ year, department, name }) => {
    if (!year) return;
  ...
  Object.entries(map).forEach(([d, ySet]) => { allowedYearsByDept[d] = Array.from(ySet).sort(); });

  return { role:"staff", allowedDepts:Object.keys(allowedYearsByDept), allowedYearsByDept, staffSubjects };
};
  ...
  return (
    <div ref={ref} style={{ position:"relative" }}>
      ...
      <button onClick={() => setOpen(o => !o)} style={{
        height:38,width:"100%",border:open?"1.5px solid #2563eb":"1px solid #cbd5e1",
        borderRadius:8,background:open?"#eff6ff":"#f8fafc",color:value?"#1e293b":"#94a3b8",
        padding:"0 10px",fontSize:13,cursor:"pointer",display:"flex",alignItems:"center",gap:6,
        boxShadow:open?"0 0 0 3px #dbeafe":"none",transition:"all 0.15s",textAlign:"left",
      }}>
      ...
      {open && (
        <div style={{ position:"absolute",top:"calc(100% + 6px)",left:0,zIndex:9999,background:"#fff",border:"1px solid #e2e8f0",borderRadius:12,boxShadow:"0 12px 40px rgba(0,0,0,0.15)",padding:14,width:260 }}>
          ...
          <div style={{ display:"grid",gridTemplateColumns:"repeat(7,1fr)",gap:2 }}>
            {cells.map((day, i) => {
              ...
              const ds = `${calYear}-${mm}-${dd}`, dow = new Date(calYear, calMonth, day).getDay();
              const isHol = !!HOLIDAY_MAP[ds], isSun = dow === 0, isSat = dow === 6, isSelected = ds === value;
              ...
              return (
                <div key={i} title={title}
                  onClick={() => { if (isHol) { alert(`${HOLIDAY_MAP[ds]} — Holiday`); return; } if (isSun) { alert("Sunday is a holiday"); return; } onChange(ds); setOpen(false); }}
                  style={{ textAlign:"center",borderRadius:5,padding:"4px 0",background:bg,color,fontWeight:fw,fontSize:11,cursor,outline:isToday?"2px solid #94a3b8":"none",outlineOffset:"-1px" }}
                >{day}</div>
              );
            })}
          </div>
          {thisMonthHolidays.length > 0 && (
            <div style={{ marginTop:10,borderTop:"1px solid #f1f5f9",paddingTop:8 }}>
              <div style={{ fontSize:10,fontWeight:700,color:"#94a3b8",marginBottom:5 }}>HOLIDAYS THIS MONTH</div>
          ...
          <div style={{ display:"flex",gap:10,marginTop:8,borderTop:"1px solid #f1f5f9",paddingTop:8,flexWrap:"wrap" }}>
            {[{bg:"#fee2e2",border:"#dc2626",label:"Holiday"},{bg:"transparent",border:"#ef4444",label:"Sunday"},{bg:"#dbeafe",border:"#3b82f6",label:"In range"}].map(l => (
...
// ─── STUDENT MODAL ───────────────────────────────────────────────────
const StudentModal = ({ student, onClose, department, year, fromDate, toDate, rawRecords, staffSubjects }) => {
  ...
  rawRecords.forEach(rec => {
    ...
    if (!entry) return;
    if (!subjectMap[subj]) subjectMap[subj] = { present:0, total:0, periods:[] };
    subjectMap[subj].total++;
    if (entry.status === "Present") subjectMap[subj].present++;
    subjectMap[subj].periods.push({
  ...
  const chartData = Object.entries(subjectMap).map(([name, d]) => ({
    name,
      ...
      <div onClick={e => e.stopPropagation()} style={{ background:"#fff",borderRadius:16,width:"100%",maxWidth:560,maxHeight:"90vh",overflowY:"auto",boxShadow:"0 24px 60px rgba(0,0,0,0.2)",animation:"slideUp 0.25s ease" }}>
        <div style={{ background:"linear-gradient(135deg,#1e3a5f,#2563eb)",borderRadius:"16px 16px 0 0",padding:"20px 24px",color:"#fff" }}>
          <div style={{ display:"flex",justifyContent:"space-between",alignItems:"flex-start" }}>
            ...
            <button onClick={onClose} style={{ background:"rgba(255,255,255,0.2)",border:"none",borderRadius:8,color:"#fff",width:32,height:32,cursor:"pointer",fontSize:16,display:"flex",alignItems:"center",justifyContent:"center" }}>✕</button>
          </div>
          <div style={{ display:"flex",gap:10,marginTop:14,flexWrap:"wrap" }}>
            {[{label:"Present",val:student.present,bg:"#16a34a"},{label:"Absent",val:student.absent,bg:"#dc2626"},{label:"Total",val:student.total,bg:"#6366f1"}].map(p => (
            ...
            <div style={{ background:"rgba(255,255,255,0.15)",borderRadius:8,padding:"6px 14px",textAlign:"center",border:`2px solid ${pctColor}` }}>
        ...
        <div style={{ padding:24 }}>
          <div style={{ display:"flex",gap:10,marginBottom:20,flexWrap:"wrap" }}>
            {[
              ...
              { bg:pct>=75?"#f0fdf4":"#fef2f2", color:pctColor, text:pct>=75?"✓ Eligible":"✗ Shortage", border:pct>=75?"#86efac":"#fca5a5" },
              { bg:"#f8fafc", color:"#475569", text:`${fromDate} → ${toDate}` },
            ].map((b, i) => (
          ...
          {chartData.length === 0 ? (
            <div style={{ textAlign:"center",padding:40,color:"#94a3b8" }}>
              ...
              No subject data found for this period
            </div>
          ) : (
            <>
              ...
              <ResponsiveContainer width="100%" height={200}>
                <BarChart data={chartData} barSize={36}>
                  ...
                  <Tooltip formatter={(val,_,props) => [`${props.payload.present}/${props.payload.total} (${val}%)`,"Attendance"]} contentStyle={{ borderRadius:8,fontSize:12,border:"1px solid #e2e8f0" }}/>
                  <Bar dataKey="percentage" radius={[6,6,0,0]}>
                    {chartData.map((e,i) => (
                      <Cell key={i} fill={e.percentage>=75?"#16a34a":e.percentage>=60?"#d97706":"#dc2626"}/>
                    ))}
                  </Bar>
                </BarChart>
              </ResponsiveContainer>
              ...
              <table style={{ width:"100%",borderCollapse:"collapse",fontSize:13 }}>
                <thead>
                  <tr style={{ background:"#f8fafc" }}>
                    {["Subject","Present","Total","%",""].map((h,hi) => (
                      <th key={hi} style={{ padding:"8px 10px",textAlign:"left",fontWeight:600,color:"#64748b",borderBottom:"1px solid #e2e8f0",fontSize:12 }}>{h}</th>
                    ))}
                  </tr>
                </thead>
                <tbody>
                    ...
                    return (
                      <React.Fragment key={i}>
                        <tr style={{ borderBottom:"1px solid #f1f5f9",cursor:"pointer",background:isEx?"#f8fafc":"transparent" }} onClick={() => setExpandedSubj(isEx ? null : row.name)}>
                          ...
                          <td style={{ padding:"9px 10px",color:"#16a34a" }}>{row.present}</td>
                          <td style={{ padding:"9px 10px",color:"#64748b" }}>{row.total}</td>
                          <td style={{ padding:"9px 10px" }}>
                            <span style={{ background:c+"18",color:c,borderRadius:4,padding:"2px 8px",fontWeight:700,fontSize:12 }}>{row.percentage}%</span>
                          </td>
...
const SubjectReport = ({ rawRecords, subject, department, year, fromDate, toDate, workingDays, onClose }) => {
  ...
  rawRecords
    ...
    .forEach(rec => {
      rec.attendance?.forEach(a => {
        ...
        if (!roll) return;
        if (!studentMap[roll]) studentMap[roll] = { roll, name, present:0, total:0, periods:[] };
        studentMap[roll].total++;
        if (a.status === "Present") studentMap[roll].present++;
        studentMap[roll].periods.push({
          date  : new Date(rec.date).toLocaleDateString("en-IN",{ day:"2-digit", month:"short" }),
          period: rec.period,
          status: a.status,
        });
  ...
  const exportSubjectPDF = () => {
    const doc = new jsPDF(), pageW = doc.internal.pageSize.getWidth();
    doc.setFillColor(124,58,237); doc.rect(0,0,pageW,28,"F");
    doc.setTextColor(255,255,255); doc.setFontSize(15); doc.setFont("helvetica","bold");
    doc.text(`SUBJECT REPORT — ${subject.toUpperCase()}`, pageW/2, 11, { align:"center" });
    doc.setFontSize(9); doc.setFont("helvetica","normal");
    doc.text(`${department} | Year ${year} | ${fromDate} to ${toDate}`, pageW/2, 21, { align:"center" });
    [{label:"Students",val:students.length},{label:"Total Classes",val:totalClasses},{label:"Class Avg",val:classAvg+"%"},{label:"Defaulters",val:defaulters.length}]
      .forEach((b,i) => {
        const x = 14 + i * 45;
        doc.setFillColor(245,243,255); doc.roundedRect(x,32,42,16,2,2,"F");
    ...
    autoTable(doc, {
      ...
      body:students.map((r,i) => [i+1,r.roll,r.name,r.present,r.absent,r.total,r.percentage+"%"]),
      styles:{ fontSize:9,cellPadding:4 },
      headStyles:{ fillColor:[124,58,237],textColor:255,fontStyle:"bold" },
      alternateRowStyles:{ fillColor:[248,250,252] },
      didParseCell:(data) => { if (data.column.index===6&&data.section==="body") { const p=parseFloat(data.cell.raw); data.cell.styles.textColor=p>=75?[22,163,74]:p>=60?[217,119,6]:[220,38,38]; data.cell.styles.fontStyle="bold"; } },
    });
    const pc = doc.internal.getNumberOfPages();
    for (let i=1;i<=pc;i++) { doc.setPage(i); doc.setFontSize(8); doc.setTextColor(150); doc.text(`Generated on ${new Date().toLocaleDateString()} | Page ${i} of ${pc}`,pageW/2,doc.internal.pageSize.getHeight()-8,{align:"center"}); }
    doc.save(`${subject}_Attendance_${department}_Year${year}.pdf`);
  };
    ...
    <div onClick={onClose} style={{ position:"fixed",inset:0,background:"rgba(0,0,0,0.6)",zIndex:99998,display:"flex",alignItems:"center",justifyContent:"center",padding:16 }}>
      <div onClick={e => e.stopPropagation()} style={{ background:"#fff",borderRadius:16,width:"100%",maxWidth:660,maxHeight:"92vh",overflowY:"auto",boxShadow:"0 24px 60px rgba(0,0,0,0.25)",animation:"slideUp 0.25s ease" }}>
        <div style={{ background:"linear-gradient(135deg,#7c3aed,#4f46e5)",borderRadius:"16px 16px 0 0",padding:"20px 24px",color:"#fff" }}>
          <div style={{ display:"flex",justifyContent:"space-between",alignItems:"flex-start" }}>
            <div>
              <div style={{ fontSize:11,opacity:0.7,marginBottom:2,letterSpacing:1 }}>SUBJECT REPORT</div>
              <div style={{ fontSize:22,fontWeight:700 }}>📚 {subject}</div>
              <div style={{ fontSize:13,opacity:0.8,marginTop:2 }}>{department} — Year {year} — {fromDate} → {toDate}</div>
            </div>
            <button onClick={onClose} style={{ background:"rgba(255,255,255,0.2)",border:"none",borderRadius:8,color:"#fff",width:32,height:32,cursor:"pointer",fontSize:16,display:"flex",alignItems:"center",justifyContent:"center" }}>✕</button>
          </div>
          <div style={{ display:"flex",gap:10,marginTop:14,flexWrap:"wrap" }}>
            {[
              { label:"Students",   val:students.length,   bg:"#4f46e5" },
              { label:"Classes",    val:totalClasses,       bg:"#0891b2" },
              { label:"Avg",        val:classAvg+"%",       bg:"#16a34a" },
              { label:"Defaulters", val:defaulters.length,  bg:"#dc2626" },
            ].map(p => (
              <div key={p.label} style={{ background:p.bg,borderRadius:8,padding:"6px 14px",textAlign:"center" }}>
                <div style={{ fontSize:18,fontWeight:700,color:"#fff" }}>{p.val}</div>
                <div style={{ fontSize:10,color:"rgba(255,255,255,0.8)" }}>{p.label}</div>
        ...
        <div style={{ padding:20 }}>
          ...
          <div style={{ display:"flex",flexDirection:"column",gap:6 }}>
            {filtered.map((r, i) => {
              ...
              return (
                <div key={r.roll} style={{ border:"1px solid #e2e8f0",borderLeft:`4px solid ${color}`,borderRadius:10,padding:"12px 16px",background:"#fff",display:"flex",justifyContent:"space-between",alignItems:"center" }}>
                  <div style={{ display:"flex",alignItems:"center",gap:12 }}>
                    <div style={{ width:32,height:32,borderRadius:8,background:"#f5f3ff",display:"flex",alignItems:"center",justifyContent:"center",fontWeight:700,color:"#7c3aed",fontSize:12,flexShrink:0 }}>{i+1}</div>
                    <div>
...
const Reports = () => {
  ...
  const [department,      setDepartment]      = useState(() => roleInfo.allowedDepts.length === 1 ? roleInfo.allowedDepts[0] : "");
  ...
  useEffect(() => {
    ...
    if (years.length === 1) setYear(years[0]); else setYear("");
    setShowReport(false); setReport([]); setRawRecords([]);
  }, [department]);
  ...
  const calculateReport = async () => {
    ...
    setLoading(true);
    try {
      const res = await axios.get(`${BASE_URL}/attendance/report`, {
  ...
  const exportPDF = () => {
    ...
    const cavgV = report.length ? (report.reduce((a,b) => a + Number(b.percentage), 0) / report.length).toFixed(1) : 0;
    const defC  = report.filter(r => r.percentage < 75).length;
    [{label:"Total Students",val:report.length},{label:"Working Days",val:workingDays||"-"},{label:"Class Average",val:cavgV+"%"},{label:"Defaulters",val:defC}]
      .forEach((b, i) => {
        const x = 14 + i * 45;
        doc.setFillColor(239,246,255); doc.roundedRect(x,32,42,16,2,2,"F");
  ...
  const exportExcel = () => {
    const data = [{Dept:department,Year:year,From:fromDate,To:toDate},{},...report.map(r => ({
      "Roll No":r.roll, Name:r.name, Present:r.present, Absent:r.absent, Total:r.total, Percentage:r.percentage+"%"
    }))];
    const ws = XLSX.utils.json_to_sheet(data), wb = XLSX.utils.book_new();
    XLSX.utils.book_append_sheet(wb, ws, "Attendance");
  ...
  const classAvg    = report.length ? (report.reduce((a,b) => a + Number(b.percentage), 0) / report.length).toFixed(1) : 0;
  const defaulters  = report.filter(r => r.percentage < 75);
  const filtered    = report.filter(r => r.roll?.toLowerCase().includes(search.toLowerCase()) || r.name?.toLowerCase().includes(search.toLowerCase()));
  ...
  const RoleBadge = () => {
    if (roleInfo.role === "admin") return <span style={{ background:"#1e3a8a",color:"#fff",borderRadius:20,padding:"4px 12px",fontSize:11,fontWeight:600 }}>🛡 Admin — All Departments</span>;
    if (roleInfo.role === "hod")   return <span style={{ background:deptCfg.color,color:"#fff",borderRadius:20,padding:"4px 12px",fontSize:11,fontWeight:600 }}>{deptCfg.emoji} HOD — {roleInfo.allowedDepts[0]}</span>;
    return (
      <div style={{ display:"flex",gap:8,flexWrap:"wrap",alignItems:"center" }}>
        ...
        {roleInfo.staffSubjects && (
          <span style={{ background:"#f5f3ff",color:"#7c3aed",borderRadius:20,padding:"4px 12px",fontSize:11,fontWeight:600,border:"1px solid #ddd8fe" }}>
            📚 Subjects: {roleInfo.staffSubjects.join(", ")}
          </span>
        )}
      </div>
    );
      ...
      <div style={{ background:"#fff",borderRadius:14,padding:20,boxShadow:"0 2px 12px rgba(0,0,0,0.07)",marginBottom:20,border:"1px solid #e2e8f0" }}>
        ...
        <div style={{ display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(140px,1fr))",gap:12,alignItems:"end" }}>
          <div>
            ...
            {roleInfo.allowedDepts.length > 1 ? (
              <select value={department} onChange={e => { setDepartment(e.target.value); setShowReport(false); setReport([]); setRawRecords([]); }} style={{ height:38,width:"100%",border:"1px solid #cbd5e1",borderRadius:8,padding:"0 10px",fontSize:13,background:"#f8fafc",color:"#1e293b" }}>
                <option value="">Select dept</option>
                {roleInfo.allowedDepts.map(d => <option key={d} value={d}>{DEPT_CONFIG[d]?.emoji} {d}</option>)}
              </select>
            ) : (
          ...
          <div>
            ...
            {allowedYears.length === 1 ? (
              <div className="readonly-pill" style={{ color:"#2563eb" }}>{YEAR_LABELS[allowedYears[0]]}</div>
            ) : (
              <select value={year} onChange={e => { setYear(e.target.value); setShowReport(false); setReport([]); setRawRecords([]); }} disabled={!department} style={{ height:38,width:"100%",border:"1px solid #cbd5e1",borderRadius:8,padding:"0 10px",fontSize:13,background:!department?"#f1f5f9":"#f8fafc",color:"#1e293b" }}>
                <option value="">Select year</option>
                {allowedYears.map(y => <option key={y} value={y}>{YEAR_LABELS[y]}</option>)}
              </select>
            )}
          </div>
        ...
        {workingDays !== null && (
          <div style={{ marginTop:14,display:"flex",gap:10,flexWrap:"wrap" }}>
            ...
            {fromDate && toDate && <div style={{ background:"#f0fdf4",borderRadius:8,padding:"6px 14px",fontSize:13,color:"#15803d",fontWeight:500,display:"inline-flex",alignItems:"center",gap:6 }}>📆 {fromDate} → {toDate}</div>}
          </div>
        )}
      </div>
