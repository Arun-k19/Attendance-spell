...
const getRoleInfo = () => {
  ...
  if (role === "admin") return { role:"admin", allowedDepts:ALL_DEPTS, allowedYearsByDept:null };

  if (role === "hod") {
    const dept = hodData?.department || userData?.department || "";
    return { role:"hod", allowedDepts: dept?[dept]:ALL_DEPTS, allowedYearsByDept:null };
  }
  ...
  return { role:"staff", allowedDepts:Object.keys(allowedYearsByDept), allowedYearsByDept };
};
...
const Reports = () => {
  ...
  const toastColors={danger:{bg:"#fee2e2",color:"#991b1b",border:"#fca5a5"},warning:{bg:"#fef9c3",color:"#854d0e",border:"#fde047"},info:{bg:"#dbeafe",color:"#1e40af",border:"#93c5fd"},success:{bg:"#dcfce7",color:"#14532d",border:"#86efac"}};
  ...
  const RoleBadge=()=>{
    ...
    return <span style={{background:"#7c3aed",color:"#fff",borderRadius:20,padding:"4px 12px",fontSize:11,fontWeight:600}}>👤 Staff — {roleInfo.allowedDepts.join(", ")}</span>;
  };
